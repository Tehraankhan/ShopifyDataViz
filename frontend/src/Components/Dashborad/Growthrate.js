...
export default function Growthrate() {
  ...
  const getData = async () => {
    try {
      ...
      const backgroundColors = data.map((item) =>
        item.growthRate <= 0 ? "#87eb98" : "#8b9deb"
      );
  ...
  const chartOptions = {
    ...
    scales: {
      x: {
        title: {
          display: true,
          text: "Monthly", // Set x-axis label dynamically based on selectedValue
      ...
      y: {
        title: {
          display: true,
          text: "Growth rate in %", // Set x-axis label dynamically based on selectedValue
          color: "black", // Label color
          font: {
            size: 16, // Increase the font size for x-axis title
          },
  ...
  return (
    <div id="Sales Growth" className="flex flex-col w-[95%] h-[600px] border-1 border-[black] rounded-[5px] mx-auto my-[70px] shadow-2xl bg-white">
      <h1 className="text-center text-[30px]">Sales Growth Rate Over Time</h1>
